...
with open('treinamento_alunos.csv', 'r', encoding='latin1') as file:
    for linha in file:
        partes = linha.strip().split(',') # Quebra a linha em partes usando vírgula como separador
...
print("2.Criamos o DataFrame a partir da lista corrigida")
...
df['mensalidade'] = (
    df['mensalidade']
    .str.replace('R$', '', regex=False) # Remove o símbolo de real
